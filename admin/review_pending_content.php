...
$content_queries = [
    ...
    "Campaigns" => "SELECT * FROM advocacy_campaigns WHERE status = 'pending'",
    "Descriptions" => "SELECT * FROM campaign_descriptions WHERE status = 'pending'",
    "Activities" => "SELECT * FROM campaign_activities WHERE status = 'pending'",
    "Images" => "SELECT * FROM campaign_images WHERE status = 'pending'",
    "Videos" => "SELECT * FROM campaign_videos WHERE status = 'pending'",
    "Infographics" => "SELECT * FROM campaign_infographics WHERE status = 'pending'",
    "Materials" => "SELECT * FROM campaign_materials WHERE status = 'pending'",
];
...
$image_columns = ['program_img', 'course_img', 'image_path', 'infographic_path', 'material_path', 'poster_path', 'campaign_img'];
$video_columns = ['video_path', 'course_video_path', 'campaign_video_path'];
...
<head>
    ...
    <link rel="stylesheet" href="../admin/assets/common/css/AdminNavBar.css">
    <link rel="stylesheet" href="../admin/assets/css/review_pending_content.css">
    <script src="../admin/assets/common/js/Toggle.js" defer></script>
</head>
<body>
                    ...
                    <tr>
                        <?php foreach ($columns as $column): ?>
                            <td class="<?php echo (strlen($row[$column]) > 30) ? 'long-text' : ''; ?>" 
                                ...
                                <?php 
                                // Separate condition for campaign_img with its own path setup for campaigns
                                } elseif (strtolower($column) === 'campaign_img' && !empty($row[$column])) {
                                    $campaign_img_path = "../campaigns/" . htmlspecialchars($row[$column]);
                                ?>
                                    <img src="<?php echo $campaign_img_path; ?>" alt="Campaign Image" class="table-image">
                                <?php 
                                // Separate condition for image_path for pending images
                                } elseif (strtolower($column) === 'image_path' && !empty($row[$column])) {
                                    $image_path = "../images/" . htmlspecialchars($row[$column]);
                                ?>
                                    <img src="<?php echo $image_path; ?>" alt="Pending Image" class="table-image">
                                <?php 
                                // Separate condition for infographic_path for pending infographics
                                } elseif (strtolower($column) === 'infographic_path' && !empty($row[$column])) {
                                    $infographic_path = "../infographics/" . htmlspecialchars($row[$column]);
                                ?>
                                    <img src="<?php echo $infographic_path; ?>" alt="Infographic" class="table-image">
                                <?php 
                                // Separate condition for material_path for pending materials (display as link)
                                } elseif (strtolower($column) === 'material_path' && !empty($row[$column])) {
                                    $material_path = "../materials/" . htmlspecialchars($row[$column]);
                                    $material_name = basename($row[$column]);
                                ?>
                                    <a href="<?php echo $material_path; ?>" target="_blank">
                                        <?php echo htmlspecialchars($material_name); ?>
                                    </a>
                                <?php 
                                ...
                                <?php 
                                    ...
                                    $file_path = "../add_add_learning_materials/" . htmlspecialchars($row[$column]);
